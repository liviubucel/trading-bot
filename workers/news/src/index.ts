...
export default {
  ...
  async syncNews(env: Env): Promise<void> {
    const now = Date.now();
    ...
    try {
      ...
      for (const ev of events) {
        await env.DB.prepare(
          "INSERT OR REPLACE INTO news_calendar (id, time, currency, eventName, impactLevel, actual, forecast, previous) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
        ).bind(
          ev.id,
          ev.time,
          ev.currency,
      ...
      await env.DB.prepare(
        ...
        "news-worker",
        "NEWS_SYNC_COMPLETE",
        `Successfully updated news calendar with ${events.length} events.`
      ).run();
      ...
      try {
        await env.DB.prepare(
          ...
          "news-worker",
          "NEWS_SYNC_FAILED",
          `News calendar update failed: ${e.message}`
        ).run();
      } catch {}
    }
  }
