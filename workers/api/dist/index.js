    ...
    async fetch(request, env) {
        ...
        try {
            ...
            if (path === "/health" || path === "/api/health") {
                ...
                return new Response(JSON.stringify({ status: "healthy", database: dbStatus ? "connected" : "error" }), { headers: corsHeaders });
            }
            // 2. OAuth Initiator
            if (path === "/oauth/url") {
            ...
            if (path === "/oauth/callback") {
                ...
                // Store in D1
                await env.DB.prepare("INSERT OR REPLACE INTO accounts (accountId, brokerName, balance, equity, isConnected, tokenData, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(accountId, brokerName, 100000.0, // default start balance
                100000.0, // default start equity
                0, // not connected yet
                JSON.stringify(tokenPayload), Date.now()).run();
                // Write Audit Log
                await env.DB.prepare("INSERT INTO audit_logs (timestamp, level, accountId, component, action, message) VALUES (?, ?, ?, ?, ?, ?)").bind(Date.now(), "INFO", accountId, "api-worker", "OAUTH_CALLBACK", `Connected account ${accountId} via OAuth flow.`).run();
                // Redirect user back to dashboard or show success
                return new Response(`<html><body style="font-family: sans-serif; text-align: center; padding: 50px; background: #0b0f19; color: #fff;">
            ...
            if (path === "/config" || path === "/api/config") {
                if (request.method === "GET") {
                    ...
                    for (const [key, val] of Object.entries(body)) {
                        await env.DB.prepare("INSERT OR REPLACE INTO config (key, valueJson, updatedAt) VALUES (?, ?, ?)").bind(key, JSON.stringify(val), Date.now()).run();
                        await env.DB.prepare("INSERT INTO audit_logs (timestamp, level, accountId, component, action, message) VALUES (?, ?, ?, ?, ?, ?)").bind(Date.now(), "WARN", "SYSTEM", "api-worker", "CONFIG_UPDATE", `Updated config key "${key}" to ${JSON.stringify(val)}`).run();
                    }
            ...
            if ((path === "/commands/execute" || path === "/api/commands/execute") && request.method === "POST") {
                ...
                const riskResponse = await env.RISK_WORKER.fetch(new Request("http://risk/evaluate", {
                    method: "POST",
                ...
                if (!safety.enableLiveTrading) {
                    ...
                    await env.DB.prepare("INSERT INTO audit_logs (timestamp, level, accountId, component, action, message, contextJson) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(Date.now(), "INFO", cmd.accountId, "api-worker", "SIMULATED_EXECUTION", `Simulated execution of ${cmd.action} for ${cmd.volume} units of ${cmd.symbol} (Live Trading Disabled)`, JSON.stringify(cmd)).run();
                    return new Response(JSON.stringify({
                        success: true,
                        status: "SIMULATED",
                ...
                const doStub = env.CTRADER_ACCOUNT_DO.get(doId);
                const executeResponse = await doStub.fetch(new Request(`http://do/command`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(cmd),
                }));
