...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method !== "POST") {
      return new Response(
        JSON.stringify({ error: "Only POST requests accepted for Telegram webhook" }),
  ...
  async handleStatus(env: Env): Promise<string> {
    ...
    const { results: accounts } = await env.DB.prepare("SELECT accountId, brokerName, isConnected FROM accounts").all<any>();
  ...
  async handleAccounts(env: Env): Promise<string> {
    const { results: accounts } = await env.DB.prepare("SELECT * FROM accounts").all<any>();
    ...
    for (const acct of accounts) {
      ...
      try {
        ...
        if (doRes.ok) {
          const doStatus: any = await doRes.json();
  ...
  async handlePositions(env: Env): Promise<string> {
    const { results: accounts } = await env.DB.prepare("SELECT accountId FROM accounts").all<any>();
    if (accounts.length === 0) return "No connected accounts found.";
    ...
    for (const acct of accounts) {
      ...
      try {
        ...
        if (doRes.ok) {
          const positions: any[] = await doRes.json();
  ...
  async handleOrders(env: Env): Promise<string> {
    const { results: accounts } = await env.DB.prepare("SELECT accountId FROM accounts").all<any>();
    if (accounts.length === 0) return "No connected accounts found.";
    ...
    for (const acct of accounts) {
      ...
      try {
        ...
        if (doRes.ok) {
          const orders: any[] = await doRes.json();
  ...
  async handleMarket(env: Env): Promise<string> {
    const { results: accounts } = await env.DB.prepare("SELECT accountId FROM accounts LIMIT 1").all<any>();
    if (accounts.length === 0) return "No registered accounts to query prices from.";
    ...
    try {
      ...
      if (doRes.ok) {
        const prices: Record<string, { bid: number; ask: number; timestamp: number }> = await doRes.json();
  ...
  async handleCalendar(env: Env): Promise<string> {
    const { results: news } = await env.DB.prepare(
      "SELECT * FROM news_calendar WHERE time >= ? ORDER BY time ASC LIMIT 5"
    ).bind(Date.now() - 3600000).all<any>(); // shows starting 1hr ago
  ...
  async handleNews(env: Env): Promise<string> {
    // Show high-impact log events or general updates
    const { results: logs } = await env.DB.prepare(
      "SELECT * FROM audit_logs WHERE component = 'news-worker' OR component = 'risk-worker' ORDER BY timestamp DESC LIMIT 5"
    ).all<any>();
  ...
  async handleHealth(env: Env): Promise<string> {
